    ...
    <section id="inner_container">
      ...
      <div class="jobdetail_wrap">
			<div class="jobdetails" style="min-height:165px;">
				...
				<a href="profile_preview.htm?companyName=${campusJobDetails.companyName}">
				
				<c:choose>
				<c:when test="${companyDetails.imageName ne null}"> 
				<div class="company_logo">
				<img src="view_image.htm?emailId=${campusJobDetails.emailId}" height="32" width="110">
				</div>
				</c:when>
				<c:otherwise>
            	...
            	<div class="details float_left">
                	 <h1 class="heading"><c:out value="${campusJobDetails.jobTitle}"/> ( Job Id <span id="iid"><c:out value="${campusJobDetails.jobId}"/></span> )</h1>
					 <h2 class="subheading"><a href="profile_preview.htm?companyName=${campusJobDetails.companyName}"><c:out value="${campusJobDetails.companyName}"/>,</a><c:out value="${campusJobDetails.location}"/></h2>
					 <h2 class="subheading">Experience required: <c:out value="${campusJobDetails.experienceFrom}"/> - <c:out value="${campusJobDetails.experienceTo}"/> years</h2>
                     <div class="jobtype"><c:out value="${campusJobDetails.jobType}"/>, <span class="postedon">Posted on:<fmt:parseDate value="${campusJobDetails.postedOn}" type="DATE" pattern="E MMM dd HH:mm:ss Z yyyy" var="postedOn"/><fmt:formatDate type="date" value="${postedOn}" pattern="dd-MMM-yyyy" /></span>
                      
                     </div>  
				     </div>              
      <c:if test="${ not fn:containsIgnoreCase(campusJobDetails.campusJobAppliedStudents,emailId) && headRole == 'STUDENT'}">         
     <div class="applybtn_wrap float_right">
     <a href="candidate_apply_broadcasted_job.htm?jobId=<c:out value="${campusJobDetails.jobIdAndFirmId}"></c:out>"><input name="" type="button" value="Apply" class="orangebuttons"></a>
     </div>
     </c:if>
              </div>
			...
			<p class="jobdescp_para"><c:out value="${campusJobDetails.jobDescription}"/></p>	
			</div>
			
			<div class="clear"></div>
             <h1 class="jobdescp_title orange_font">Salary Per Week ($)</h1>
			<div class="jobdescp_para_wrap">
			<p class="jobdescp_para"> <c:out value="${campusJobDetails.payPerWeek}"/></p>
			</div>
			...
			<div class="clear"></div>
             <h1 class="jobdescp_title orange_font">Industry</h1>
			<div class="jobdescp_para_wrap">
			<p class="jobdescp_para"> <c:out value="${campusJobDetails.industry}"/></p>
			</div>
			
			<div class="clear"></div>
             <h1 class="jobdescp_title orange_font">Gpa Cut-Off</h1>
			<div class="jobdescp_para_wrap">
			<p class="jobdescp_para"> <c:out value="${campusJobDetails.gpaCutOffFrom}"/> -  <c:out value="${campusJobDetails.gpaCutOffTo}"/></p>
			</div>
