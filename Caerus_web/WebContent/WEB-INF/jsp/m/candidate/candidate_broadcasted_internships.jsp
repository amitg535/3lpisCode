...
<!--[if gt IE 9]><!--><html class="no-js" lang="en"><!--<![endif]-->
        ...
        <title>Internships From Campus</title>
		...
		<div id="mid_wrap" class="midwrap_toppadding">
            ...
            <c:set var="broadcastedCount" value="${broadcastedCount + 1}" scope="page"/>
				...
				<h1 class="page_heading">
					...
					<c:if test="${msg!=null}">
						<div class="margin20_top margin20_bottom">You've Successfully applied to this internship!</div>
						</c:if>
				<c:choose>
					<c:when test="${count == 0}">
					...
					<c:otherwise>
							...
							<c:forEach items="${employerInternshipListForUniversity}" var="internshipDetails">
							<c:if test="${internshipDetails.status=='Broadcasted'}">
									<li onclick = "location.href='campus_internship_preview.htm?internshipId=${internshipDetails.internshipIdAndFirmId}&&universityName=${internshipDetails.universityName}'"><a href="#">
										<c:choose>
											<c:when test="${internshipDetails.photoName ne null}"> 
											<div class="company_logo">
											<a href="profile_preview.htm?companyName=${internshipDetails.firmName}">
                   							<img src="view_image.htm?emailId=${internshipDetails.firmId}"/></a>
											</div>
											</c:when>
											<c:otherwise>
											...
											<h1 class="heading">
												<a href="campus_internship_preview.htm?internshipId=${internshipDetails.internshipIdAndFirmId}&&universityName=${internshipDetails.universityName}">
												<c:out value="${internshipDetails.internshipTitle}" /></a>
											</h1>
											<h2 class="subheading">
												<a href="profile_preview.htm?companyName=${internshipDetails.firmName}"><c:out value="${internshipDetails.firmName}"></c:out></a>
												,
												<c:out value="${internshipDetails.internshipLocation}" />
											</h2>
											<div class="jobtype">
												<span class="postedon">Posted on 
												<fmt:parseDate value="${internshipDetails.postedOn}" type="DATE" pattern="E MMM dd HH:mm:ss Z yyyy" var="postedOn"/>
